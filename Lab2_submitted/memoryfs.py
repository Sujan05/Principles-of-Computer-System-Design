import pickle, logging, unicodedata
...
## returns number of bytes written
  ...
  # -------Added by Sujan for assignment 1-----
  def Read(self, file_inode_number, offset, count):
       ...
       while bytes_read < count:
           ...
           #read_start = current_offset
           if (offset + count) >= next_block_boundary:
             # the data length is such that it goes beyond this block, so we're writing this entire block
             read_end = BLOCK_SIZE
           else:
             # otherwise, the data is truncated within this block
           ...
           if block_number < DATA_BLOCKS_OFFSET:
               logging.debug ("Read: not a data block")
           else:
               #block = file_inode.RawBlocks.Get(block_number)
               block = self.RawBlocks.Get(block_number)
               data += block[read_start:read_end].decode()
           current_offset += read_end-read_start
           bytes_read += read_end-read_start
           logging.debug ('Read: current_offset: ' + str(current_offset) + ' , bytes_read: ' + str(bytes_read) + ' , count: ' + str(count))

       return data

  #----Added by Sujan for assignment 2----
  def Plain_name(self, path):
      for i in range(len(path)):
          if(path[i]=="/"):
              return False
      return True

  def first_name(self, path):
      splitted_pathname = path.split('/')
      return splitted_pathname[0]

  def rest_name(self, path):
      splitted_pathname = path.split('/')
      added_pathname = ""
      for i in range(len(splitted_pathname)-1):
          if (i == 0):
              added_pathname = added_pathname + splitted_pathname[i+1]
          else:
              added_pathname = added_pathname + "/" + splitted_pathname[i+1]
      return added_pathname

  #path name implementation
  def PathToInodeNumber(self, path, dir):
      if (self.Plain_name(path)==True):
          filename_inode = self.Lookup(path, dir)
          if filename_inode == -1:
              print("Error: File not found\n")
          return filename_inode
      else:
          dir =  self.Lookup(self.first_name(path), dir)
          if dir == -1:
              print("Error: File not found\n")
              return -1
          path =  self.rest_name(path)
          return self.PathToInodeNumber(path, dir)

  #Absolute path name implementation
  def GeneralPathToInodeNumber(self, path, cwd):
      if(path[0]=="/"):
          pathname_length = len(path)
          new_pathname = path[1:pathname_length]
          return self.PathToInodeNumber(new_pathname,0)
      else:
          return self.PathToInodeNumber(path,cwd)

  #implementation of Link function
  def Link(self, target, name, cwd):
      target_file_inode = self.GeneralPathToInodeNumber(target, 0)
      filename_exist = False
      #print(name)
      #print(len(name))
      if (target_file_inode != -1):
          inobj = InodeNumber(self.RawBlocks, target_file_inode)
          inobj.InodeNumberToInode()
          if inobj.inode.type == INODE_TYPE_FILE:  # Here, it has been checked that the target is a file
              #print("the target is a file\n")
              empty_size = self.FindAvailableFileEntry(cwd)
              #print(empty_size)
              if (empty_size == -1):    # Here is the check for available entry
                  print("Error: No space available for the entry\n")
              elif(empty_size < MAX_FILE_SIZE):
                  inobj2 = InodeNumber(self.RawBlocks, cwd)
                  inobj2.InodeNumberToInode()
                  block_index = 0
                  while block_index <= (inobj2.inode.size // BLOCK_SIZE):
                    block = self.RawBlocks.Get(inobj2.inode.block_numbers[block_index])
                    if block_index == (inobj2.inode.size // BLOCK_SIZE):
                      end_position = inobj2.inode.size % BLOCK_SIZE
                    else:
                    ...
                    while current_position < end_position:
                      ...
                      inobj3 = InodeNumber(self.RawBlocks, entryinodenumber)
                      inobj3.InodeNumberToInode()
                      if inobj3.inode.type != INODE_TYPE_DIR:
                        found_name = entryname.decode().replace("\x00","")
                        #print(found_name)
                        if (found_name == name):
                            filename_exist = True
                  ...
                  if(filename_exist == True):    # Here, it has been checked whether the file exists or not
                      print("Error: File exists in the directory.\n")
                  else:
                      if (len(name) > MAX_FILENAME):
                          print("Error: Name is greater than maximum file name size\n")
                      else:
                          self.InsertFilenameInodeNumber(inobj2, name, target_file_inode)
              empty_size = self.FindAvailableFileEntry(cwd)
              #print(empty_size)
          else:
              print("Error: The target is a directory\n")
      return 0
