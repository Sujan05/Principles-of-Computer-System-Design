...
class DiskBlocks():
  def __init__(self, server_url):
    ...
    self.block_server = []
    for i in server_url:
        server = xmlrpc.client.ServerProxy(i, use_builtin_types=True)
        self.block_server.append(server)
    #self.block_server1 = xmlrpc.client.ServerProxy(server_url[0], use_builtin_types=True)
    #self.block_server2 = xmlrpc.client.ServerProxy(server_url[1], use_builtin_types=True)
## Put: interface to write a raw block of data to the block indexed by block number
## Blocks are padded with zeroes up to BLOCK_SIZE

  def matrix_gen(self, raid_block_number):
    NUM_SERVER = len(self.block_server)
    matrix = [[0 for x in range(NUM_SERVER)] for y in range(NUM_SERVER)]
    array = [0 for x in range(NUM_SERVER*(NUM_SERVER-1))]
    for i in range(NUM_SERVER*(NUM_SERVER-1)):
        array[i] = i
      #print(array)
    k = NUM_SERVER-1
    sum = NUM_SERVER-1
    ...
    for i in range(len(array)):
        ...
        matrix[x][y]=array[i]
      #print(matrix)
    position = [(index, row.index(raid_block_number)) for index, row in enumerate(matrix) if raid_block_number in row]
      #print(position[0][1])
    return position

  def mapping(self, raid_block_number):
    NUM_SERVER = len(self.block_server)
    norm_raid_block_number = raid_block_number % (NUM_SERVER*(NUM_SERVER-1))
    mul_factor = raid_block_number // (NUM_SERVER*(NUM_SERVER-1))
    position = self.matrix_gen(norm_raid_block_number)
    #print(position)
    block_server_id = position[0][1]
    if ((position[0][0]+position[0][1]) > (NUM_SERVER - 1)):
        block_number = (position[0][0] - 1) + (mul_factor * (NUM_SERVER-1))
    else:
        block_number = position[0][0] + (mul_factor * (NUM_SERVER-1))
    parity_server_id = NUM_SERVER - 1 - position[0][0]
    tuple = [block_server_id, block_number, parity_server_id]
    return tuple


  def Put(self, raid_block_number, block_data):

    #print(len(self.block_server))
    Tuple = self.mapping(raid_block_number)
    server_id = Tuple[0]
    block_number = Tuple[1]
    parity_id = Tuple[2]
    #print(raid_block_number)
    #print(server_id)
    #print(block_number)
    #print(parity_id)
    #print(len(self.block_server))
    ...
    m = hashlib.md5()
    block_data_string = str(block_data)
    block_data_bytes = bytes(block_data_string,'utf-8')
    m.update(block_data_bytes)
    checksum = bytearray(m.digest())
    #print(m.digest())
    #print(m.digest_size)
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      ret = self.block_server[server_id].Put(block_number,putdata, checksum)
      if ret == -1:
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def Get(self, raid_block_number):
...
## RSM: read and set memory equivalent
  ...
  def RSM(self, raid_block_number):
